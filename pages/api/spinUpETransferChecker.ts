      ...
      connection.once("ready", () => {
        ...
        connection.openBox("INBOX", (err, box) => {
          ...
          connection.search(searchCriteria, (err, results) => {
            if (err) {
              console.log(
                "-------------- nothing matching the search criteria found"
              );
              // console.error(err);
              return;
            }
                ...
                simpleParser(stream, async (err, parsed) => {
                  ...
                  if (text) {
                    ...
                    if (matches) {
                      ...
                      if (status == "cancelled") {
                        console.log(
                          "Order is already cancelled - stopping cron job"
                        );
                        cronJob.stop();
                        return;
                      }
                      ...
                      try {
                        await Promise.all([
                          ...
                          WooCommerce.post(`orders/${id}/notes`, {
                            note:
                              "E-Transfer payment received - Reference Number: " +
                              referenceNumber,
                          }),
                        ]);
                        cronJob.stop();
